...
function formatConsoleEvent(event) {
    switch (event.type) {
        ...
        case "command.invalid":
            return `invalid command scope=${event.scope} index=${event.index}${event.name ? ` name=${formatName(event.name)}` : ""}`;
        case "batch.skipped":
        ...
        case "command.spawned":
            return `spawned scope=${event.scope} index=${event.index} name=${formatName(event.name)}${event.pid === undefined ? "" : ` pid=${event.pid}`}`;
...
function formatFileEvent(event) {
    switch (event.type) {
        ...
        case "command.invalid":
            return `${event.type} scope=${event.scope} index=${event.index}${event.name ? ` name=${formatName(event.name)}` : ""}`;
        case "batch.skipped":
        ...
        case "command.spawned":
            return `${event.type} scope=${event.scope} index=${event.index} name=${formatName(event.name)}${event.pid === undefined ? "" : ` pid=${event.pid}`}`;
        case "command.spawn-failed":
        case "command.child-error":
...
function getDefaultLogPath(options) {
...
function appendEvent(filePath, timestamp, event) {
    ...
    appendFileSync(filePath, `${timestamp.toISOString()} ${formatFileEvent(event)}\n`, "utf8");
}
